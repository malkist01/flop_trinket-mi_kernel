...
int dsi_display_param_store(struct dsi_display *display, uint32_t param)
{
	...
	case DISPPARAM_HBM_FOD_ON:
		...
		if (panel->samsung_flag) {
			rc = dsi_panel_set_dimming_brightness(
				panel, HBM_OFF_DIMMING_OFF,
				panel->last_bl_lvl);
			rc = dsi_panel_set_dimming_brightness(
				panel, HBM_ON_DIMMING_OFF,
				panel->bl_config.bl_max_level);
		} else
			rc = dsi_panel_set_dimming_brightness(
				panel, HBM_OFF_DIMMING_OFF,
	...
	case DISPPARAM_HBM_FOD_OFF:
		...
		if ((display->drm_dev &&
		    ...
		    (display->drm_dev &&
		     display->drm_dev->sde_power_mode == SDE_MODE_DPMS_LP2)) {
			...
			if ((display->drm_dev &&
			    ...
			    (display->drm_dev &&
			     display->drm_dev->sde_power_mode ==
				     SDE_MODE_DPMS_LP2)) {
				if (panel->samsung_flag)
					rc = dsi_panel_set_doze_backlight(
						panel, AOD_BRIGHTNESS);
				else
					rc = dsi_panel_set_doze_backlight(
						panel, AOD_BRIGHTNESS * 2);
				if (rc)
					pr_err("failed to set doze mode backlight,rc=%d\n",
					       rc);
			} else {
				if (panel->last_bl_lvl && panel->samsung_flag) {
					...
					if (rc)
						pr_err("failed to set backlight,rc=%d\n",
						       rc);
				} else if (panel->last_bl_lvl &&
					   (!panel->samsung_flag)) {
					rc = dsi_panel_set_brightness(
						panel, HBM_OFF_DIMMING_OFF,
						panel->last_bl_lvl);
					if (rc)
						pr_err("failed to set backlight,rc=%d\n",
					  ...
					  const char *buf, size_t len)
{
	...
	case 0x1: /* cabc_movie on */
		dsi_panel_set_feature(display->panel,
