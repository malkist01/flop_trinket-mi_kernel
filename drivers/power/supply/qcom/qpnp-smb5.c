...
union power_supply_propval lct_therm_call_level = {
    5,
};
union power_supply_propval lct_therm_globe_level = {
    2,
};
union power_supply_propval lct_therm_india_level = {
    1,
};
...
#define PMI632_MAX_ICL_UA (mi_is_laurel() ? 2000000 : 3000000)
...
static int smb5_chg_config_init(struct smb5 *chip) {
  ...
  case PMI632_SUBTYPE:
    ...
    chg->wa_flags |= WEAK_ADAPTER_WA | USBIN_OV_WA | CHG_TERMINATION_WA |
                     USBIN_ADC_WA | SKIP_MISC_PBS_IRQ_WA;
...
static int smb5_parse_dt(struct smb5 *chip) {
  ...
  if (!rc &&
      (chip->dt.chg_inhibit_thr_mv < 0 || chip->dt.chg_inhibit_thr_mv > 300)) {
  ...
  chg->fake_chg_status_on_debug_batt =
      of_property_read_bool(node, "qcom,fake-chg-status-on-debug-batt");
  ...
  chg->dcin_uusb_over_gpio_en =
      of_property_read_bool(node, "qcom,dcin-uusb-over-gpio-en");

  if (chg->dcin_uusb_over_gpio_en) {
    chg->micro_usb_gpio = of_get_named_gpio(node, "qcom,micro-usb-gpio", 0);
    if (!gpio_is_valid(chg->micro_usb_gpio)) {
      pr_err(" micro_usb_gpio not specified\n");
    } else {
      rc = devm_gpio_request(chg->dev, chg->micro_usb_gpio, "micro_usb");
      if (rc)
      ...
      rc = devm_request_threaded_irq(
          chg->dev, chg->micro_usb_irq, NULL, smb_micro_usb_irq_handler,
          IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
          "micro_usb_irq", chg);
      if (rc < 0)
        dev_err(chg->dev, "Unable to request micro_usb_irq: %dn", rc);
    ...
    if (!gpio_is_valid(chg->dc_9v_gpio)) {
      ...
      rc = devm_gpio_request(chg->dev, chg->dc_9v_gpio, "dc_9v");
      if (rc)
      ...
      rc = devm_request_threaded_irq(
          chg->dev, chg->dc_9v_irq, NULL, smb_micro_usb_irq_handler,
          IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
          "dc_9v_irq", chg);
      if (rc < 0)
        dev_err(chg->dev, "Unable to request dc_9v_irq: %dn", rc);
      enable_irq_wake(chg->dc_9v_irq);
    }

    chg->usb_switch_gpio = of_get_named_gpio(node, "qcom,usb-switch-gpio", 0);

    if (!gpio_is_valid(chg->usb_switch_gpio)) {
      pr_err("usb_switch_gpio not specified\n");
    } else {
      rc = devm_gpio_request(chg->dev, chg->usb_switch_gpio, "usb_switch");
      if (rc)
    ...
    chg->usb_hub_33v_en_gpio =
        of_get_named_gpio(node, "qcom,usb-hub-33v-en-gpio", 0);

    if (!gpio_is_valid(chg->usb_hub_33v_en_gpio)) {
      pr_err("usb_hub_33v_en_gpio not specified\n");
    } else {
      rc = devm_gpio_request(chg->dev, chg->usb_hub_33v_en_gpio,
                             "usb_hub_33v_en");
      if (rc)
...
static int
smb5_set_prop_comp_clamp_level(struct smb_charger *chg,
...
static int smb5_usb_set_prop(struct power_supply *psy,
  ...
  case POWER_SUPPLY_PROP_THERM_ICL_LIMIT:
    if (!is_client_vote_enabled(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER)) {
      ...
      icl = get_client_vote(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER) +
            val->intval;
    }
...
static int smb5_init_usb_psy(struct smb5 *chip) {
  ...
  if (IS_ENABLED(CONFIG_MACH_XIAOMI_C3J) && mi_is_ginkgo())
    chg->usb_psy =
        devm_power_supply_register(chg->dev, &chg->usb_psy_desc, &usb_cfg);
  else
    chg->usb_psy =
        devm_power_supply_register(chg->dev, &usb_psy_desc, &usb_cfg);
...
static int smb5_init_usb_port_psy(struct smb5 *chip) {
  ...
  chg->usb_port_psy =
      devm_power_supply_register(chg->dev, &usb_port_psy_desc, &usb_port_cfg);
...
static int smb5_usb_main_set_prop(struct power_supply *psy,
  ...
  case POWER_SUPPLY_PROP_FLASH_ACTIVE:
    if ((chg->chg_param.smb_version == PMI632_SUBTYPE) &&
      ...
      if (pval.intval) {
        rc = smblib_force_vbus_voltage(chg, chg->flash_active ? FORCE_5V_BIT
                                                              : IDLE_BIT);
        if (rc < 0)
...
static int smb5_init_usb_main_psy(struct smb5 *chip) {
  ...
  chg->usb_main_psy =
      devm_power_supply_register(chg->dev, &usb_main_psy_desc, &usb_main_cfg);
...
static int smb5_batt_get_prop(struct power_supply *psy,
  ...
  case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
    rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_TIME_TO_FULL_NOW, val);
    break;
...
static int smb5_batt_set_prop(struct power_supply *psy,
  ...
  case POWER_SUPPLY_PROP_SET_SHIP_MODE:
    ...
    if (!val->intval)
      break;
    if (chg->pl.psy)
      power_supply_set_property(chg->pl.psy, POWER_SUPPLY_PROP_SET_SHIP_MODE,
                                val);
...
static int smb5_init_vconn_regulator(struct smb5 *chip) {
  ...
  chg->vconn_vreg->rdev =
      devm_regulator_register(chg->dev, &chg->vconn_vreg->rdesc, &cfg);
...
static int smb5_configure_typec(struct smb_charger *chg) {
  ...
  rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
                           EN_TRY_SNK_BIT | EN_SNK_ONLY_BIT, EN_TRY_SNK_BIT);
  if (rc < 0) {
  ...
  if (IS_ENABLED(CONFIG_MACH_XIAOMI_F9S) && mi_is_laurel()) {
    rc = smblib_masked_write(chg, TYPE_C_DEBUG_ACCESS_SINK_REG,
                             TYPEC_DEBUG_ACCESS_MODE_MASK, 0x17);
    if (rc < 0) {
      dev_err(chg->dev, "Couldn't configure 0x154A rc=%d\n", rc);
      return rc;
    }
  }
...
static int smb5_configure_mitigation(struct smb_charger *chg) {
  ...
  rc = smblib_masked_write(
      ...
      THERMREG_SW_ICL_ADJUST_BIT | THERMREG_DIE_ADC_SRC_EN_BIT |
          ...
          SKIN_ADC_CFG_BIT | THERMREG_CONNECTOR_ADC_SRC_EN_BIT,
      src_cfg);
  if (rc < 0) {
...
static int smb5_init_hw(struct smb5 *chip) {
  ...
  if (IS_ENABLED(CONFIG_MACH_XIAOMI_F9S) && mi_is_laurel()) {
    ...
    rc = smblib_write(chg, AICL_RERUN_TIME_CFG_REG, AICL_RERUN_TIME_180S_VAL);
  } else {
  ...
  if (chip->dt.auto_recharge_vbat_mv != -EINVAL) {
    ...
    rc = smblib_batch_write(chg, CHGR_ADC_RECHARGE_THRESHOLD_MSB_REG,
                            (u8 *)&temp, 2);
    if (rc < 0) {
  ...
  if (IS_ENABLED(CONFIG_MACH_XIAOMI_C3J) && mi_is_ginkgo()) {
    ...
    if (rc < 0) {
      dev_err(chg->dev,
              "Couldn't configure USBIN_9V_AICL_THRESHOLD_REG rc=%d\n",
              rc);
      return rc;
    }
     ...
     */
    rc = smblib_masked_write(
        chg, TYPE_C_DEBUG_ACC_SNK_CFG,
        TYPEC_DEBUG_ACC_SNK_SEL_ICL | TYPEC_DEBUG_ACC_SNK_DIS_AICL |
            TYPEC_DEBUG_ENABLE_ACC_SNK | TYPEC_DEBUG_ENABLE_CHG_ON_SNK,
        TYPEC_DEBUG_ACC_SNK_SEL_ICL | TYPEC_DEBUG_ENABLE_ACC_SNK |
...
static int smb5_request_interrupt(struct smb5 *chip, struct device_node *node,
  ...
  rc = devm_request_threaded_irq(chg->dev, irq, NULL,
                                 smb5_irqs[irq_index].handler, IRQF_ONESHOT,
                                 irq_name, irq_data);
  if (rc < 0) {
...
static void smb5_create_debugfs(struct smb5 *chip) {
  ...
  file = debugfs_create_file("force_batt_psy_update", 0600, chip->dfs_root,
                             chip, &force_batt_psy_update_ops);
  if (IS_ERR_OR_NULL(file))
    pr_err("Couldn't create force_batt_psy_update file rc=%ld\n", (long)file);

  file = debugfs_create_file("force_usb_psy_update", 0600, chip->dfs_root, chip,
                             &force_usb_psy_update_ops);
  if (IS_ERR_OR_NULL(file))
    pr_err("Couldn't create force_usb_psy_update file rc=%ld\n", (long)file);

  file = debugfs_create_file("force_dc_psy_update", 0600, chip->dfs_root, chip,
                             &force_dc_psy_update_ops);
  if (IS_ERR_OR_NULL(file))
...
static struct device_attribute attrs2[] = {
    __ATTR(thermalcall, S_IRUGO | S_IWUSR, lct_thermal_call_status_show,
           lct_thermal_call_status_store),
};
...
static int smb5_show_charger_status(struct smb5 *chip) {
  ...
  pr_debug_ratelimited("SMB5 status - usb:present=%d type=%d batt:present = %d "
                       "health = %d charge = %d\n",
                       usb_present, chg->real_charger_type, batt_present,
...
static int smb5_probe(struct platform_device *pdev) {
  ...
  if (IS_ENABLED(CONFIG_MACH_XIAOMI_C3J) && mi_is_ginkgo()) {
    for (attr_count2 = 0; attr_count2 < ARRAY_SIZE(attrs2); attr_count2++) {
      rc = sysfs_create_file(&chg->dev->kobj, &attrs2[attr_count2].attr);
      if (rc < 0) {
...
static void smb5_shutdown(struct platform_device *pdev) {
  ...
  if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC)
    smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK,
                        EN_SNK_ONLY_BIT);
