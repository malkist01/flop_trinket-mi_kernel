...
int smblib_get_iio_channel(struct smb_charger *chg, const char *propname,
  ...
  rc =
      of_property_match_string(chg->dev->of_node, "io-channel-names", propname);
  if (rc < 0)
    return 0;
...
int smblib_get_jeita_cc_delta(struct smb_charger *chg, int *cc_delta_ua) {
  ...
  if (stat & BAT_TEMP_STATUS_HOT_SOFT_BIT) {
    ...
    rc = smblib_get_charge_param(chg, &chg->param.jeita_cc_comp_cold,
                                 &cc_minus_ua);
    if (rc < 0) {
      smblib_err(chg, "Couldn't get jeita cc minus rc=%d\n", rc);
      return rc;
...
static int smblib_select_sec_charger_locked(struct smb_charger *chg,
  ...
  case POWER_SUPPLY_CHARGER_SEC_CP:
    ...
    rc = smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, EN_CP_CMD_BIT,
                             EN_CP_CMD_BIT);
    if (rc < 0) {
  ...
  case POWER_SUPPLY_CHARGER_SEC_PL:
    ...
    rc = smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, EN_STAT_CMD_BIT,
                             EN_STAT_CMD_BIT);
    if (rc < 0) {
      dev_err(chg->dev, "Couldn't enable SMB charger rc=%d\n", rc);
      return rc;
...
static const struct apsd_result smblib_apsd_results[] = {
    [UNKNOWN] = {.name = "UNKNOWN", .bit = 0, .pst = POWER_SUPPLY_TYPE_UNKNOWN},
    [SDP] = {.name = "SDP",
             .bit = SDP_CHARGER_BIT,
...
static const struct apsd_result *
smblib_get_apsd_result(struct smb_charger *chg) {
  int rc, i;
  u8 apsd_stat, stat;
...
void smblib_apsd_enable(struct smb_charger *chg, bool enable) {
  ...
  rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, BC1P2_SRC_DETECT_BIT,
...
static const struct apsd_result *
smblib_update_usb_type(struct smb_charger *chg) {
...
static void smblib_uusb_removal(struct smb_charger *chg) {
  ...
  rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, (u8)FLOAT_OPTIONS_MASK,
                           chg->float_cfg);
  if (rc < 0)
...
int smblib_toggle_smb_en(struct smb_charger *chg, int toggle) {
  ...
  rc = smblib_select_sec_charger(chg, chg->sec_chg_selected, chg->cp_reason,
                                 true);
...
void batt_temp_charging_current(struct smb_charger *chg, int bat_temp) {
  ...
  if (bat_current != chg->last_bat_current) {
    pr_err("bat_current = %d,bat_temp = %d,last_bat_current=%d\n", bat_current,
           bat_temp, chg->last_bat_current);
    rc = vote(chg->fcc_votable, BATT_TEMP_VOTER, true, bat_current);
    if (rc < 0) {
...
static int smblib_set_moisture_protection(struct smb_charger *chg,
  ...
  if (enable) {
    ...
    if (rc < 0) {
      smblib_err(chg,
                 "Couldn't enable moisture detection and uUSB state change "
                 "interrupt rc=%d\n",
                 rc);
      return rc;
    }
...
static int smblib_chg_disable_vote_callback(struct votable *votable, void *data,
  ...
  rc =
      smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG, CHARGING_ENABLE_CMD_BIT,
...
int smblib_vconn_regulator_disable(struct regulator_dev *rdev) {
  ...
  rc =
      smblib_masked_write(chg, TYPE_C_VCONN_CONTROL_REG, VCONN_EN_VALUE_BIT, 0);
  if (rc < 0)
...
int smblib_get_prop_batt_iterm(struct smb_charger *chg,
  ...
  if (chg->chg_param.smb_version == PMI632_SUBTYPE)
    ...
    temp =
        DIV_ROUND_CLOSEST(temp * ITERM_LIMITS_PM8150B_MA, ADC_CHG_ITERM_MASK);
...
int smblib_set_prop_system_temp_level(struct smb_charger *chg,
  ...
  if (mi_is_ginkgo()) {
    ...
    if ((lct_backlight_off) && (LctIsInCall == 0) && (val->intval > 2)) {
      pr_info_ratelimited("level ignored: backlight_off:%d level:%d",
    ...
    if ((LctIsInCall == 1) && (val->intval != 5)) {
      pr_info_ratelimited("level ignored: LctIsInCall:%d level:%d", LctIsInCall,
                          val->intval);
      return 0;
    }
  }
  ...
  if (bypass_charging) {
    ...
    if (system_temp_level < 0)
      system_temp_level = 0;
    pr_info_ratelimited("%s limited charging enabled %d", __FUNCTION__,
                        system_temp_level);
  } else if (system_temp_level > 0) {
    pr_info_ratelimited("%s charging enabled, but thermal limited %d",
                        __FUNCTION__, system_temp_level);
  }
...
static int smblib_update_thermal_readings(struct smb_charger *chg) {
  ...
  rc = smblib_read_iio_channel(chg, chg->iio.die_temp_chan, DIV_FACTOR_DECIDEGC,
                               &chg->die_temp);
  if (rc < 0) {
  ...
  if (chg->sec_chg_selected == POWER_SUPPLY_CHARGER_SEC_CP) {
    if (is_cp_available(chg)) {
      rc = power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_DIE_TEMP,
                                     &pval);
      if (rc < 0) {
    ...
    rc = power_supply_get_property(chg->pl.psy, POWER_SUPPLY_PROP_CHARGER_TEMP,
                                   &pval);
    if (rc < 0) {
...
out:
  smblib_dbg(chg, PR_MISC,
             "Current temperatures: \tDIE_TEMP: %d,\tCONN_TEMP: %d,\tSMB_TEMP: "
             "%d,\tSKIN_TEMP: %d\nTHERMAL_STATUS: %d\n",
             chg->die_temp, chg->connector_temp, chg->smb_temp, chg->skin_temp,
             thermal_status);
...
int smblib_get_prop_dc_online(struct smb_charger *chg,
  ...
  val->intval =
      (stat & USE_DCIN_BIT) && (stat & VALID_INPUT_POWER_SOURCE_STS_BIT);
...
int smblib_get_prop_usb_online(struct smb_charger *chg,
  ...
  val->intval =
      (stat & USE_USBIN_BIT) && (stat & VALID_INPUT_POWER_SOURCE_STS_BIT);
  return rc;
}
...
static int smblib_estimate_adaptor_voltage(struct smb_charger *chg,
  ...
  case POWER_SUPPLY_TYPE_USB_PD:
    ...
    val->intval =
        chg->voltage_min_uv + ((chg->voltage_max_uv - chg->voltage_min_uv) / 2);
    break;
  default:
    val->intval = MICRO_5V;
    break;
...
bool smblib_rsbux_low(struct smb_charger *chg, int r_thr) {
  ...
  rc =
      smblib_masked_write(chg, TYPE_C_CRUDE_SENSOR_CFG_REG,
  ...
  if (mi_is_ginkgo()) {
    if (stat & (SNK_RP_STD_DAM_BIT | SNK_RP_1P5_DAM_BIT | SNK_RP_3P0_DAM_BIT)) {
      ...
      smblib_masked_write(chg, TYPE_C_DEBUG_ACC_SNK_CFG,
                          ...
                          TYPEC_DEBUG_ENABLE_CHG_ON_SNK);
      smblib_masked_write(chg, SCHG_USB_TYPE_C_CFG, BC1P2_START_ON_CC, 0);
    }
  }
...
static int smblib_handle_usb_current(struct smb_charger *chg, int usb_current) {
  ...
  if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
    ...
    if (chg->dcin_uusb_over_gpio_en && gpio_is_valid(chg->dc_9v_gpio) &&
    ...
    if (mi_is_ginkgo()
            ...
            : ((usb_current < SDP_CURRENT_UA) && is_flash_active(chg)))
      usb_current = SDP_CURRENT_UA;
...
int smblib_set_prop_sdp_current_max(struct smb_charger *chg,
  ...
  if (!chg->pd_active) {
    ...
    if (val->intval <= USBIN_25MA)
      rc = vote(chg->usb_icl_votable, PD_SUSPEND_SUPPORTED_VOTER, true,
                val->intval);
    else
...
int smblib_set_prop_typec_power_role(struct smb_charger *chg,
  ...
  if (rc < 0) {
    smblib_err(
        chg, "Couldn't write 0x%02x to TYPE_C_INTRPT_ENB_SOFTWARE_CTRL rc=%d\n",
        power_role, rc);
    goto unlock;
  }
...
int smblib_set_prop_typec_select_rp(struct smb_charger *chg,
  ...
  if (val->intval < TYPEC_SRC_RP_MAX_ELEMENTS) {
    rc = smblib_masked_write(chg, TYPE_C_CURRSRC_CFG_REG, TYPEC_SRC_RP_SEL_MASK,
                             val->intval);
    if (rc < 0)
...
int smblib_set_prop_pd_active(struct smb_charger *chg,
  ...
  smblib_usb_pd_adapter_allowance_override(chg, !!chg->pd_active ? FORCE_5V
                                                                 : FORCE_NULL);
  smblib_update_usb_type(chg);
  power_supply_changed(chg->usb_psy);
  return rc;
...
static void smblib_eval_chg_termination(struct smb_charger *chg,
                                        u8 batt_status) {
  union power_supply_propval pval = {
      0,
  };
...
static void smblib_usbov_delay_report_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, usbov_delay_report_work.work);
...
void smblib_usb_plugin_locked(struct smb_charger *chg) {
  ...
  if (vbus_rising) {
    ...
    if (need_confirm &&
        attach_time - detach_time < DETACH_ATTACH_MAX_INTERVAL) {
    ...
    if (chg->collapsed) {
      detach_time = jiffies;
      smblib_err(chg,
                 "detached after collapse --> time = %lu, collapse-time = %lu, "
                 "last_attach_time = %lu\n",
    ...
    if (chg->wa_flags & WEAK_ADAPTER_WA) {
      ...
      smblib_set_charge_param(chg, &chg->param.aicl_5v_threshold,
                              chg->aicl_5v_threshold_mv);
      smblib_set_charge_param(chg, &chg->param.aicl_cont_threshold,
  ...
  if (apsd_result->bit & QC_3P0_BIT) {
    ...
    if (!chg->apsd_ext_timeout && !timer_pending(&chg->apsd_timer)) {
      ...
      mod_timer(&chg->apsd_timer,
                msecs_to_jiffies(APSD_EXTENDED_TIMEOUT_MS) + jiffies);
    }
  }
...
static void smblib_handle_hvdcp_check_timeout(struct smb_charger *chg,
  ...
  if (rising) {
    ...
    if (qc_charger) {
      ...
      vote(chg->usb_icl_votable, DCP_VOTER, chg->dcp_icl_ua != -EINVAL,
           chg->dcp_icl_ua);
    }
  }
...
static void update_sw_icl_max(struct smb_charger *chg, int pst) {
  ...
  case POWER_SUPPLY_TYPE_USB:
    ...
    if (!is_client_vote_enabled(chg->usb_icl_votable, USB_PSY_VOTER)) {
      ...
      if (chg->dcin_uusb_over_gpio_en && gpio_is_valid(chg->dc_9v_gpio) &&
          gpio_get_value(chg->dc_9v_gpio))
        vote(chg->usb_icl_votable, GPIO_DCIN_VOTER, true,
             is_flash_active(chg) ? SDP_CURRENT_UA : DCP_CURRENT_UA);
      else
        /* if flash is active force 500mA */
  ...
  case POWER_SUPPLY_TYPE_USB_FLOAT:
    ...
    if (chg->dcin_uusb_over_gpio_en && gpio_is_valid(chg->dc_9v_gpio) &&
        gpio_get_value(chg->dc_9v_gpio))
      vote(chg->usb_icl_votable, GPIO_DCIN_VOTER, true, DCP_CURRENT_UA);
    else
      /*
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data) {
  ...
  smblib_handle_hvdcp_check_timeout(chg, (bool)(stat & HVDCP_CHECK_TIMEOUT_BIT),
...
static void typec_src_insertion(struct smb_charger *chg) {
  ...
  chg->ok_to_pd =
      (!(chg->typec_legacy || *chg->pd_disabled) || chg->early_usb_attach) &&
      !chg->pd_not_supported;
...
irqreturn_t smb_micro_usb_irq_handler(int irq, void *data) {
  ...
  int is_dc_9v_present =
      (gpio_is_valid(chg->dc_9v_gpio) && gpio_get_value(chg->dc_9v_gpio));
  ...
  if (is_usb_present) {
    ...
    if (is_dc_9v_present && chg->micro_usb_pre_state) {
  ...
  schedule_delayed_work(&chg->micro_usb_switch_work, msecs_to_jiffies(1000));
...
int smblib_set_prop_pr_swap_in_progress(struct smb_charger *chg,
  ...
  rc = smblib_masked_write(
      chg, TYPE_C_EXIT_STATE_CFG_REG, BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT,
  ...
  if (chg->pr_swap_in_progress) {
    ...
    rc = smblib_masked_write(chg, TYPE_C_CCOUT_CONTROL_REG, TYPEC_CCOUT_SRC_BIT,
                             0);
    if (rc < 0) {
      smblib_err(chg, "Couldn't read TYPE_C_CCOUT_CONTROL_REG rc=%d\n", rc);
      return rc;
...
static void smblib_micro_usb_switch_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, micro_usb_switch_work.work);
  int is_usb_present = (gpio_is_valid(chg->micro_usb_gpio) &&
                        gpio_get_value(chg->micro_usb_gpio));
  int is_dc_9v_present =
      (gpio_is_valid(chg->dc_9v_gpio) && gpio_get_value(chg->dc_9v_gpio));

  if (!gpio_is_valid(chg->micro_usb_gpio) && !gpio_is_valid(chg->dc_9v_gpio)) {
  ...
  if (is_usb_present) {
    ...
    if (is_dc_9v_present && (chg->real_charger_type == POWER_SUPPLY_TYPE_USB))
...
static void smblib_pr_swap_detach_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, pr_swap_detach_work.work);
  int rc;
  u8 stat;
...
static void pl_update_work(struct work_struct *work) {
  ...
  rc = power_supply_set_property(chg->pl.psy,
                                 POWER_SUPPLY_PROP_CHARGER_TEMP_MAX, &prop_val);
  if (rc < 0) {
...
static void smblib_thermal_regulation_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, thermal_regulation_work.work);
  int rc;
...
static void usb_current_worker(struct work_struct *work) {
  ...
  struct smb_charger *chip =
      container_of(dwork, struct smb_charger, usb_current_work);
...
static void period_update(struct work_struct *work) {
  struct delayed_work *dwork = to_delayed_work(work);
  struct smb_charger *chip =
      container_of(dwork, struct smb_charger, period_update_work);
  union power_supply_propval val = {
      0,
  };
  ...
  if ((period_ms >= 500) || (old_cap != cap) || (old_status != status) ||
      ...
      (printk_counter >= 5)) {
    pr_err("***temp=%d,v_batt=%dmV,c_batt=%dmA,cap=%d,status=%d,charge_type=%d,"
           "batt_p=%d,usb_p=%d,usb_v=%dmV,chg_done=%d\n",
           temp, v_batt, c_batt, cap, status, charge_type, batt_p, usb_p, usb_v,
           chip->chg_done);
...
static void smblib_moisture_protection_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, moisture_protection_work);
  int rc;
  bool usb_plugged_in;
  u8 stat;
  ...
  if (stat == U_USB_STATUS_WATER_PRESENT || stat == U_USB_FMB1_BIT ||
      stat == U_USB_FMB2_BIT || (usb_plugged_in && stat == U_USB_FLOAT1_BIT)) {
...
static enum alarmtimer_restart moisture_protection_alarm_cb(struct alarm *alarm,
                                                            ktime_t now) {
  struct smb_charger *chg =
      container_of(alarm, struct smb_charger, moisture_protection_alarm);
...
static void smblib_chg_termination_work(struct work_struct *work) {
  ...
  rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
  if (rc < 0)
    goto out;
...
static void jeita_update_work(struct work_struct *work) {
  ...
  if (!pnode && IS_ENABLED(CONFIG_MACH_XIAOMI_F9S) && mi_is_laurel()) {
  ...
  if (!rc) {
    ...
    rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-hys-thresholds",
...
static void smblib_dual_role_check_work(struct work_struct *work) {
  struct smb_charger *chg =
      container_of(work, struct smb_charger, role_reversal_check.work);
  int rc = 0;
  ...
  case DUAL_ROLE_PROP_MODE_UFP:
    if (chg->typec_mode < POWER_SUPPLY_TYPEC_SOURCE_DEFAULT) {
      smblib_dbg(chg, PR_MISC,
                 "Role reversal not latched to UFP in %d msecs. Resetting to "
                 "DRP mode\n",
  ...
  case DUAL_ROLE_PROP_MODE_DFP:
    if (chg->typec_mode >= POWER_SUPPLY_TYPEC_SOURCE_DEFAULT ||
      ...
      smblib_dbg(chg, PR_MISC,
                 "Role reversal not latched to DFP in %d msecs. Resetting to "
                 "DRP mode\n",
                 ROLE_REVERSAL_DELAY_MS);
      chg->pr_swap_in_progress = false;
...
static int smblib_create_votables(struct smb_charger *chg) {
  ...
  chg->dc_suspend_votable = create_votable(
      "DC_SUSPEND", VOTE_SET_ANY, smblib_dc_suspend_vote_callback, chg);
  ...
  chg->awake_votable =
      create_votable("AWAKE", VOTE_SET_ANY, smblib_awake_vote_callback, chg);
  ...
  chg->chg_disable_votable = create_votable(
      "CHG_DISABLE", VOTE_SET_ANY, smblib_chg_disable_vote_callback, chg);
...
int smblib_init(struct smb_charger *chg) {
  ...
  if (chg->uusb_moisture_protection_enabled) {
    ...
    if (alarmtimer_get_rtcdev()) {
      alarm_init(&chg->moisture_protection_alarm, ALARM_BOOTTIME,
  ...
  rc = qcom_step_chg_init(chg->dev, chg->step_chg_enabled,
              ...
              IS_ENABLED(CONFIG_MACH_XIAOMI_F9S) &&
                mi_is_laurel());
