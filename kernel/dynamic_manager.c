...
static void do_save_dynamic_manager(struct callback_head *_cb)
{
	...
	if (ksu_kernel_write_compat(fp, &tw->config, sizeof(tw->config),
				    &off) != sizeof(tw->config)) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0)
			strscpy(config->hash, dynamic_manager.hash,
				sizeof(config->hash));
#else
			strlcpy(config->hash, dynamic_manager.hash,
				sizeof(config->hash));
#endif
			ret = 0;
		} else {
